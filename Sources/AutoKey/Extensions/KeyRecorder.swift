...
/// A custom control that records a key press with optional modifiers
class KeyRecorder: NSControl {
    ...
    var keyCode: CGKeyCode? = nil { // No default key
        didSet {
            updateDisplay()
    ...
    private func handleKeyEvent(_ event: NSEvent) {
        ...
        if event.type == .keyDown {
            ...
            // Update modifiers when they change
            self.modifierFlags = carbonModifiersFrom(event.modifierFlags)
        }
    }
    
    private func isValidKey(_ keyCode: CGKeyCode) -> Bool {
